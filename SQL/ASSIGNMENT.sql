...
-- 8 -- 
SELECT SUM(SCOST*SOLD) AS REVENUE FROM SOFTWARE WHERE DEV_IN = 'C';
...
-- QUERY IV --
-- 1 --
SELECT SALARY, GROUP_CONCAT(NAME) FROM PROGRAMMER WHERE SALARY IN ( SELECT SALARY FROM PROGRAMMER GROUP BY SALARY HAVING COUNT(*) > 1) GROUP BY SALARY ORDER BY SALARY;

-- 2 --
SELECT S.NAME, S.TITLE,P.SEX,P.SALARY FROM SOFTWARE S JOIN PROGRAMMER P ON P.NAME = S.NAME WHERE P.SEX='M' AND P.SALARY >3000;

-- 3 --
SELECT S.* FROM SOFTWARE S JOIN PROGRAMMER P ON S.NAME=P.NAME WHERE P.SEX='F' AND S.DEV_IN='PASCAL';
...
-- 6 --
SELECT S.NAME, ST.SPLACE, COUNT(S.TITLE),SUM(S.SOLD), SUM(S.SOLD * S.SCOST) FROM SOFTWARE S 
JOIN PROGRAMMER P ON S.NAME = P.NAME
JOIN STUDIES ST ON S.NAME = ST.NAME
GROUP BY S.NAME, ST.SPLACE;

-- 7 --
SELECT S.* FROM SOFTWARE S 
JOIN PROGRAMMER P ON S.NAME = P.NAME
JOIN STUDIES ST ON ST.NAME = S.NAME
...
-- 13 --
SELECT SUM(S.SCOST) FROM SOFTWARE S 
JOIN STUDIES ST ON S.NAME = ST.NAME
WHERE ST.SPLACE = 'APPLE';

-- 14 --
SELECT DOJ, GROUP_CONCAT(NAME) FROM PROGRAMMER GROUP BY DOJ HAVING COUNT(*) > 1;

-- 15 --
SELECT PROF2, GROUP_CONCAT(NAME) FROM PROGRAMMER GROUP BY PROF2 HAVING COUNT(*) > 1;

-- 16 --
SELECT ST.SPLACE, SUM(S.SCOST*S.SOLD) AS TOTAL FROM SOFTWARE S
JOIN STUDIES ST ON S.NAME = ST.NAME
GROUP BY ST.SPLACE;

-- 17 --
SELECT ST.SPLACE, P.NAME FROM STUDIES ST 
JOIN PROGRAMMER P ON ST.NAME = P.NAME
JOIN SOFTWARE S ON ST.NAME = S.NAME
...
-- 19 --
SELECT S.NAME,P.SALARY, ST.COURSE FROM SOFTWARE S 
JOIN PROGRAMMER P ON S.NAME = P.NAME
JOIN STUDIES ST ON S.NAME = ST.NAME
...
-- 20 --
SELECT P.NAME, CEIL(ST.CCOST/P.SALARY)  AS TIMENEEDED FROM PROGRAMMER P
JOIN STUDIES ST ON P.NAME=ST.NAME;
...
SELECT S.* FROM SOFTWARE S 
JOIN PROGRAMMER P ON S.NAME = P.NAME 
WHERE FLOOR(DATEDIFF(CURDATE(), P.DOJ) / 365) < 3 
ORDER BY S.DCOST DESC LIMIT 1;
...
-- 24 --
SELECT COUNT(S.TITLE) AS TOTAL, ST.SPLACE, P.NAME 
FROM SOFTWARE S
JOIN PROGRAMMER P ON S.NAME = P.NAME
JOIN STUDIES ST ON P.NAME = ST.NAME
WHERE S.SCOST = (
...
JOIN PROGRAMMER P ON S.NAME = P.NAME
JOIN STUDIES ST ON P.NAME = ST.NAME
WHERE FLOOR(DATEDIFF(CURDATE(), P.DOJ)) = (
    SELECT MAX(FLOOR(DATEDIFF(CURDATE(), DOJ)))
    FROM PROGRAMMER
) AND ST.SPlace = 'BDPS';
...
-- 28 --
SELECT P.PROF1, COUNT(DISTINCT P.NAME) AS NumberOfProgrammers, COUNT(S.TITLE) AS NumberOfPackages
FROM PROGRAMMER P
LEFT JOIN SOFTWARE S ON P.PROF1 = S.DEV_IN
GROUP BY P.PROF1;

-- 29 --
SELECT S.NAME,COUNT(DEV_IN) FROM PROGRAMMER P, SOFTWARE S WHERE P.NAME = S.NAME GROUP BY S.NAME;
 
-- 30 --
SELECT P.* FROM PROGRAMMER P 
JOIN STUDIES ST ON P.NAME=ST.NAME
WHERE ST.SPLACE = 'COIT'; -- ST.PLACE='S.S.I.L';
